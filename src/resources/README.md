...
This document contains a brief description of the project layout and instructions for running the *logns-gen* data generator. Users that are not familiar with the specifics of running Myriad-based data generators should read the following text in order to understand how the parallel data generation process can be configured and initiated.

We assume that the reader has already followed the *Build and Install* instructions from the `README.md` file located in the root of the source project and has installed the *logns-gen* data generator under the install path `<logns-gen-install>`.

In the following discussion, we refer to the set of core components comprising the Myriad toolkit as *toolkit* and to the set of toolkit extensions comprising the *logns-gen* data generator as *logns-gen extensions* or *generator extensions*.


Manually Starting Generator Nodes
=================================

If you look at the contents of the `<logns-gen-install>/bin` folder, you will find two files -- `logns-gen-frontend` and `logns-gen-node`. 
...
Once the values for these parameters are specified, you can start node `<i>` of your `<N>`-node parallel generation setup manually by typing:
    ...
    <logns-gen-install>$ bin/logns-gen-node -m<DATASET_ID> -s<SF> -i<i> -N<N> -o<PATH>
...
If you want to use the Python parallel deployment frontend, you have to setup your parallel environment. For this purpose, you need to create a `logns-gen-frontend.xml` configuration inside the `<logns-gen-install>/config` folder (you can use `logns-gen-config.template.xml` bundled during the build process as a basis). The Python frontend config files has the following structure:
...
For instance, consider a scenario where you have a 10 node cluster but only want to use the first five nodes for the data generation process. This will translate into the following `<cloud_env>` entry:
    ...
    <cloud-env id="mycloud-05" dgen="<logns-gen-install>">
      <master>
...
Now suppose that at a later point you want to use all 10 nodes. You can add a second `<cloud-env>` to reflect this: 
    ...
    <cloud-env id="mycloud-10" dgen="<logns-gen-install>">
      <master>
        <host name="myhost01.example.com" />
      </master>
      <slaves>
...
* The `<logns-gen-install>` should be is accessible from all nodes (this is best achieved through NFS exported install path).
...
We recommend choosing a `nodes-per-host` value in the range [X,2X] where X is the number of processor cores in each host in the referenced environment. You can start the parallel deployment script for a particular <DATASET_ID> and scaling factor <SF> with this command:
    ...
    <logns-gen-install>$ bin/logns-gen-frontend -m<DATASET_ID> -s<SF> <DGEN-CONFIG-ID>
